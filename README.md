...
## Данные, разметка и метрики

Для сегментации талька мы вручную подготовили разметку в Label Studio: экспертные контуры областей оталькования были экспортированы в JSON, затем конвертированы в бинарные PNG-маски и сопоставлены с исходными микрофотографиями. Эта разметка используется только для класса `talc`, потому что для сульфидных фаз полноценной пиксельной разметки в данных не было.

Датасет классификации собран в 3 класса:

```text
ordinary  - 540 изображений
talc      - 126 изображений
difficult - 458 изображений
```

Метрики сегментации талька на 3-fold валидации для `UnetPlusPlus + efficientnet-b3`, `img_size=768`:

| Fold | Best epoch | Val IoU | Val Dice |
| ---: | ---: | ---: | ---: |
| 0 | 35 | 0.4965 | 0.6431 |
| 1 | 11 | 0.5949 | 0.7252 |
| 2 | 23 | 0.5445 | 0.6962 |
...
После 3-fold проверки финальная сегментационная модель была дообучена на всех 42 размеченных масках талька и экспортирована в ONNX.

Лучший результат классификатора `EfficientNet-B3`, `img_size=512`, 3 класса:

| Metric | Value |
| --- | ---: |
| Val accuracy | 0.9200 |
| Val balanced accuracy | 0.9105 |
| Val macro F1 | 0.9057 |
| Best epoch | 20 |
